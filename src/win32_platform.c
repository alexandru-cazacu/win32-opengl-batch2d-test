...
internal float hy_get_cpu_load()
{
  ...
  return GetSystemTimes(&idleTime, &kernelTime, &userTime)
             ? hy_calc_cpu_load(hy_file_time_to_int64(idleTime),
                                hy_file_time_to_int64(kernelTime) + hy_file_time_to_int64(userTime))
             : -1.0f;
}
...
internal void win32_handle_compositionchanged(HyWindow* data)
{
  ...
  if (enabled) {
    ...
    DwmSetWindowAttribute(data->window, DWMWA_NCRENDERING_POLICY, &(DWORD){DWMNCRP_ENABLED}, sizeof(DWORD));
  }
...
internal LRESULT win32_handle_nchittest(HyWindow* data, int x, int y)
{
  ...
  if ((mouse.x < data->width) && (mouse.x >= 0) && (mouse.y < (50)) && (mouse.y >= 0)) {
    return HTCAPTION;
  }
...
internal void win32_handle_windowposchanged(HyWindow* data, const WINDOWPOS* pos)
{
  ...
  if (client_changed) {
    ...
    if (data->width > old_width) {
      InvalidateRect(data->window, &(RECT){old_width - 1, 0, old_width, old_height}, TRUE);
    } else {
      InvalidateRect(data->window, &(RECT){data->width - 1, 0, data->width, data->height}, TRUE);
    }
    if (data->height > old_height) {
      InvalidateRect(data->window, &(RECT){0, old_height - 1, old_width, old_height}, TRUE);
    } else {
      InvalidateRect(data->window, &(RECT){0, data->height - 1, data->width, data->height}, TRUE);
    }
  }
}

internal LRESULT win32_handle_message_invisible(HWND window, UINT msg, WPARAM wparam, LPARAM lparam)
{
...
///
internal bool hy_window_should_close(HyWindow* window)
{
...
internal int hy_window_create_borderless(HyWindow* hyWindow, HyWindowStartMode startMode, const char* title)
{
  ...
  ATOM cls = RegisterClassExW(&(WNDCLASSEXW){.cbSize = sizeof(WNDCLASSEXW),
  ...
  hyWindow->window = CreateWindowExW(WS_EX_APPWINDOW | WS_EX_LAYERED, (LPWSTR)MAKEINTATOM(cls), L"Hyper",
